    ...
    for tree in new_trees:
        ...
        for config in configs:
            ...
            if signal is not None and ~signal:
                # Remove filters that are only meant for signal [SignalParticleX, SignalParticleY, SignalParticleZ]
                signal_keys = []
                for key in params.keys():
                    if "SignalParticle" in key[1]:
                        signal_keys.append(key)
                for key in signal_keys:
                    # output += f"[cyan][INFO][/cyan] Omitting filter {key} for background run.\n"
                    params.pop(key)
                output += f"[cyan][INFO][/cyan] Omitted {len(signal_keys)} signal-only filters for background run.\n"

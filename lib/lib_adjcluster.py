...
from lib.lib_format import remove_branches, get_param_dict, reshape_array
...
def compute_adjcl_basics(
    ...
    new_branches = [
        ...
        "MeanAdjClR",
        "MeanAdjClTime",
        "TotalAdjClSameGenCharge",
        "MaxAdjClSameGenCharge",
        "MeanAdjClSameGenCharge",
    ]

    radial_limits = np.arange(20, 120, 20)
    new_radial_int_branches = [
        "TotalAdjClNum",
        "TotalAdjClSameGenNum",
        "TotalAdjClIntrinsicBkgNum",
        "TotalAdjClExternalBkgNum",
    ]
    new_radial_float_branches = [
        "TotalAdjClCharge",
        "TotalAdjClSameGenCharge",
        "TotalAdjClIntrinsicBkgCharge",
        "TotalAdjClExternalBkgCharge",
        "TotalAdjClRatio",
    ]
    ...
    for config in configs:
        ...
        for limit in radial_limits:
            run["Reco"][f"{new_radial_int_branches[1]}{limit}"][idx] = np.sum(
                (run["Reco"]["AdjClR"][idx] < limit)
                * (run["Reco"]["AdjClR"][idx] > 0)
                * gen_idx,
                axis=1,
            )
        ...
        for limit in radial_limits:
            rad_idx = (run["Reco"]["AdjClR"][idx] < limit) * (
                run["Reco"]["AdjClR"][idx] > 0
            )
    ...
    for limit in radial_limits:
        ...
        run["Reco"][f"TotalAdjClIntrinsicBkgNum{limit}"] = (
        ...
        run["Reco"][f"TotalAdjClRatio{limit}"] = (
            run["Reco"][f"TotalAdjClSameGenNum{limit}"]
            / (np.sum(
                run["Reco"][f"TotalAdjClExternalBkgNum{limit}"]
                + run["Reco"][f"TotalAdjClIntrinsicBkgNum{limit}"],
        ...
        # Substitute all non finite values with 0
        run["Reco"][f"TotalAdjClRatio{limit}"] = np.where(
            (run["Reco"][f"TotalAdjClRatio{limit}"] == np.inf)
            + (run["Reco"][f"TotalAdjClRatio{limit}"] == -np.inf)
            + (run["Reco"][f"TotalAdjClRatio{limit}"] == np.nan)
            + (run["Reco"][f"TotalAdjClSameGenNum{limit}"] == 0)
            + ((run["Reco"][f"TotalAdjClExternalBkgNum{limit}"] == 0)
            + (run["Reco"][f"TotalAdjClIntrinsicBkgNum{limit}"] == 0)),
            0,
            run["Reco"][f"TotalAdjClRatio{limit}"],
        )
...
def compute_adjcl_advanced(
    ...
    for config in configs:
        ...
        run["Reco"]["AdjCldTime"][idx] = (
            run["Reco"]["AdjClTime"][idx] - converted_array_time
        )
        run["Reco"]["AdjClRelCharge"] = (
            run["Reco"]["AdjClCharge"][idx] / converted_array_charge
        )
        run["Reco"]["AdjClChargePerHit"] = (
            run["Reco"]["AdjClCharge"][idx] / run["Reco"]["AdjClNHits"][idx]
        )

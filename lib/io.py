...
from .filters import compute_filtered_run
...
def merge_with_existing_df(
    ...
    """
    Preserve columns from an existing results pkl that are absent in new_df.

    Called before save_df when only a subset of metrics was recomputed.
    Rows are matched on key_cols (e.g. NHits, OpHits, AdjCl).  Columns
    present in the existing file but NOT in new_df are left-joined back in,
    so previously computed metric results survive a partial rerun.

    Returns new_df (possibly enriched with preserved columns).
    """
    filepath, _, _ = prepare_file_save(
        ...
        subfolder=subfolder,
        filename=filename,
        rm=False,        # don't delete — we want to read it
        filetype="pkl",
        debug=False,
    )
    ...
    try:
        ...
        rprint(f"[yellow][WARNING][/yellow] merge_with_existing_df: could not load {filepath}: {exc}")
        return new_df

    if existing.empty:
        return new_df

    preserve = [c for c in existing.columns if c not in new_df.columns]
    if not preserve:
        return new_df  # nothing to restore

    valid_keys = [k for k in (key_cols or []) if k in existing.columns and k in new_df.columns]
    if not valid_keys:
        rprint(f"[yellow][WARNING][/yellow] merge_with_existing_df: no matching key columns in {filepath}; skipping merge")
        return new_df

    try:
        merged = new_df.merge(
            existing[valid_keys + preserve],
            on=valid_keys,
            how="left",
        )
    except Exception as exc:
        rprint(f"[yellow][WARNING][/yellow] merge_with_existing_df: merge failed for {filepath}: {exc}")
        return new_df

    if debug:
        rprint(f"[cyan][INFO][/cyan] Preserved {len(preserve)} column(s) from existing results: {preserve}")
    return merged
        ...
        for tree in root_info["TreeNames"]:
            ...
            for branch in track(root_info[tree], description=f"Processing {tree}..."):
                if branch in done_root_info[tree.split(";")[0]]:
                    if debug:
                        rprint(
                            f"[yellow]Branch {branch} already processed for tree {tree}. Skipping...[/yellow]"
                        )
                    continue
                print_colored(
                ...
                if branch == "SelectedEvents":
                    # Find the total number of entries in the branch. The brnach is made of lists of variable length
                    lengths = ak.num(this_array, axis=1)
                    save2pnfs(
                        path + name + "/" + out_folder + "/AnalyzedEvents.npy",
                        user_input,
                        lengths,
                        debug,
                    )
...
def array2list(array, debug=False):
    ...
    if type(array) == np.ndarray:
        ...
        try:
            ...
            rprint("[red]ERROR: Cannot convert awkward array to list[/red]")
            raise None
    else:
                ...
                for key in branches_dict[tree]:
                    ...
                    if key == "Generator":
                        ...
                        try:
                            ...
                            run[tree]["GeneratorLabel"] = np.asarray(
                                ["Unknown"] * len(branch), dtype=str
                            )
...
def get_gen_color(names: list[str], debug=False):
    ...
    for name in names:
        if simple_names_list[name] == "Unknown":
            color_dict[name] = colors[4]
        elif simple_names_list[name].lower() in ["marley", "neutrino", "solar"]:
            color_dict[name] = colors[6]
        elif simple_names_list[name].lower() in ["apa", "crp"]:
            color_dict[name] = colors[8]
        elif simple_names_list[name] == "Gamma":
            color_dict[name] = "black"
        elif simple_names_list[name] == "Neutron":
            color_dict[name] = colors[3]
        elif simple_names_list[name].lower() in ["cpa", "cathode"]:
            ...
            color_dict[name] = colors[2]
        elif simple_names_list[name].lower() == "hep":
            color_dict[name] = colors[7]
        elif simple_names_list[name].lower() == "8b":
            color_dict[name] = colors[6]
        else:
            color_dict[name] = colors[4]
...
def get_simple_names(names: list[str], debug: bool = False) -> dict:
    ...
    for name in names:
        ...
        elif "crp" in name.lower():
            simple_names[name] = "CRP"
        elif "apa" in name.lower():
            simple_names[name] = "APA"
        elif "pds" in name.lower():

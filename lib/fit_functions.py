...
def get_hist2d_fit(
    ...
    if spec_type == "top+bottom":
        x_spec, y_top, y_bottom = spectrum_hist2d(
            x, y, h, threshold, spec_type=spec_type, debug=debug
        )
        top_func, raw_labels, top_popt, top_perr = fit_hist1d(
            x_spec, y_top, trimm=trimm, func=func_type, debug=debug
        )
        if debug:
            fig.add_trace(
                go.Scatter(
        ...
        fig.add_trace(
            go.Scatter(
                x=x_spec,
                y=top_func(x_spec, *top_popt),
                mode="lines",
                marker=dict(color="grey"),
                name="Top Fit",
                error_y=dict(
                    type="data", array=top_func(x_spec, *top_perr), visible=True
                ),
            ),
            row=row,
            col=col,
        )
        bottom_func, labels, bottom_popt, bottom_perr = fit_hist1d(
            x_spec, y_bottom, trimm=trimm, func=func_type, debug=debug
        )
        if debug:
            fig.add_trace(
                go.Scatter(
        ...
        fig.add_trace(
            go.Scatter(
                x=x_spec,
                y=bottom_func(x_spec, *bottom_popt),
                mode="lines",
                marker=dict(color="grey"),
                name="Bottom Fit",
                error_y=dict(
                    type="data", array=bottom_func(x_spec, *bottom_perr), visible=True
                ),
            ),
            row=row,
            col=col,
        ...
        fig.update_layout(
            xaxis=dict(range=[np.min(x_spec), np.max(x_spec)]),
            yaxis=dict(range=[np.min(y_bottom), np.max(y_top)]),
        )
        ...
        x_spec, y_spec = spectrum_hist2d(
            x, y, h, threshold, spec_type=spec_type, debug=debug
        )
        func, labels, popt, perr = fit_hist1d(
            x_spec, y_spec, trimm=trimm, func=func_type, debug=debug
        )
        if debug:
            fig.add_trace(
                go.Scatter(
        ...
        fig.update_layout(
            xaxis=dict(range=[np.min(x_spec), np.max(x_spec)]),
            yaxis=dict(range=[np.min(y_spec), np.max(y_spec)]),
        )
    ...
    rprint("[blue]INFO: " + "".join(debug_text) + "[/blue]")
...
def get_hist1d_fit(x, acc, fig, row, col, trimm=5, func_type="gauss", debug=False):
    ...
    if type(acc) == np.ndarray or type(acc) == list:
        ...
        print_colored(
            "ValueError: acc must be an integer, float, numpy array, or list!",
            "WARNING",
        )
        return fig, [], []
    ...
    x = (x[1:] + x[:-1]) / 2
    fig.add_trace(
        go.Bar(x=x, y=h, marker=dict(color="grey"), name="Spectrum"), row=row, col=col
    )
    fig.update_layout(bargap=0)

    try:
        func, labels, popt, perr = fit_hist1d(
            x, h, trimm=trimm, func=func_type, debug=debug
        )

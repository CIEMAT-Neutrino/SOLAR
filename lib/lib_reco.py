...
from .lib_adjcluster import compute_adjcl_basics, compute_adjcl_advanced
from .lib_adjopflash import compute_adjopflash_basics
from .lib_ophit import compute_ophit_basic, compute_ophit_event
from .lib_opflash import (
...
from .lib_drift import compute_true_drift
...
def compute_reco_workflow(
    ...
    if any(work in workflow for work in ["TRUTH", "OPFLASH"]):
    ...
    ### CALIBRATION AND RECONSTRUCTION WORKFLOWS
        ...
        if [
            a in workflow
            for a in ["RECONSTRUCTION", "SMEARING", "ANALYSIS", "SIGNIFICANCE"]
        ].count(True) > 0:
        ...
        if [a in workflow for a in ["SMEARING", "ANALYSIS", "SIGNIFICANCE"]].count(
            True
        ) > 0:

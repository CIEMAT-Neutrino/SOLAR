...
from lib.lib_format import get_param_dict, remove_branches
...
def compute_signal_directions(
    ...
    new_branches = [
        ...
        f"TSignalDirectionY",
        f"TSignalDirectionZ",
        f"TSignalDirectionMod",
    ]
    for tree in trees:
        for branch in new_branches:
            run[tree][branch] = np.zeros(
                (len(run[tree]["Event"]), len(run[tree][f"TSignalPDG"][0])),
                dtype=np.float32,
            )
        ...
        for config in configs:
            ...
            for direction, start, end in zip(
                [f"TSignalDirectionX", f"TSignalDirectionY", f"TSignalDirectionZ"],
                ["SignalParticleX", "SignalParticleY", "SignalParticleZ"],
                [f"TSignalEndX", f"TSignalEndY", f"TSignalEndZ"],
            ):
                run[tree][direction][idx] = (
                    run[tree][start][:, None][idx] - run[tree][end][idx]
                )

            run[tree][f"TSignalDirectionMod"][idx] = np.sqrt(
                np.power(run[tree][f"TSignalDirectionX"][idx], 2)
                + np.power(run[tree][f"TSignalDirectionY"][idx], 2)
                + np.power(run[tree][f"TSignalDirectionZ"][idx], 2)
            )
            for coord in ["X", "Y", "Z"]:
                run[tree][f"TSignalDirection{coord}"][idx] = (
                    run[tree][f"TSignalDirection{coord}"][idx]
            ...
            run[tree][f"TSignalPhi"][idx] = np.arctan2(
                run[tree][f"TSignalDirectionY"][idx],
                run[tree][f"TSignalDirectionX"][idx],
            )
...
def compute_signal_energies(
    ...
    for tree in trees:
        pdg_list = np.unique(
            run[tree][f"TSignalPDG"][np.where(run[tree][f"TSignalMother"] == 0)]
        )
...
def compute_particle_energies(
    ...
    """
    output += f"\t[magenta][LOG][/magenta] Computing MC particle energies...\n"
    ...
    for tree in trees:
        for branch in required_branches[tree]:
            if branch not in run[tree]:
                output += f"\t\t[yellow][WARNING][/yellow] Missing branch {branch} in tree {tree}. Skipping particle energy computation.\n"
                output += f"\tParticle energy computation \t-> Skipped!\n\n"
                return run, output, []
    ...
    for config, tree in product(configs, trees):
        ...
        for particle in particles_pdg:
            if particles_pdg[particle] not in run[tree][f"TSignalPDG"][idx]:
                rprint(
                    f"[yellow]Warning:[/yellow] Particle {particle} with PDG {particles_pdg[particle]} not found in {tree} tree."
                )
                continue

            run[tree][f"{particle}K"][idx] = np.sum(
                run[tree][f"TSignalK"][idx]
                * np.array(run[tree][f"TSignalPDG"][idx] == particles_pdg[particle])
                * np.array(run[tree][f"TSignalMother"][idx] == 0),
                axis=1,
            )
    ...
    output += f"\tTrue particle energy computation \t-> Done!\n\n"
    return run, output, new_branches

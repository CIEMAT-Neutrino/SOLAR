...
def _precompute_osc_result(dm2: float, sin13: float, sin12: float, backend: str):
    ...
    e_bins  = analysis_info.get("OSC_ENERGY_BINS",  analysis_info.get("ENERGY_BINS",  120))
    e_range = analysis_info.get("OSC_ENERGY_RANGE", analysis_info.get("ENERGY_RANGE", [0, 30]))
...
def _load_osc_pkl_as_pee(
    info: dict, dm2: float, sin13: float, sin12: float
) -> "tuple[np.ndarray, np.ndarray, np.ndarray]":
    """
    Load the raw oscillation pkl for (dm2, sin13, sin12) and return
    (pee_2d, n_centers, nadir_pdf) where pee_2d[i,j] = plain P_ee(nadir_i, E_j).

    The raw pkl stores P_ee × nadir_pdf (output of combine_day_night).
    ...
    osc_path = f"{info['PATH']}/data/OSCILLATION/"
    pkl_path = (
        f"{osc_path}pkl/rebin/"
        f"osc_probability_dm2_{dm2:.3e}_sin13_{sin13:.3e}_sin12_{sin12:.3e}.pkl"
    )
    raw_df    = pd.read_pickle(pkl_path)
    n_centers = raw_df.index.values  # cos_nadir bin centers

    # Re-derive nadir_pdf so we can normalise day/night subsets correctly
    try:
        nadir_pdf = get_nadir_pdf_file(path=osc_path, nadir_centers=n_centers)
    except Exception:
        nadir_pdf = get_nadir_pdf_nufast(n_centers)

    # raw_df.values[i, j] = P_ee(nadir_i, E_j) × nadir_pdf[i]  (from combine_day_night)
    with np.errstate(divide="ignore", invalid="ignore"):
        pee_2d = np.where(
            nadir_pdf[:, np.newaxis] > 0,
            raw_df.values / nadir_pdf[:, np.newaxis],
            0.0,
        )

    return pee_2d, n_centers, nadir_pdf
...
def _compute_osc_kde_and_exposure(
    ...
    the nadir bin centers and normalised nadir PDF.  Works for all three backends:
      - prob3/nufast: caller extracts pee_2d from OscResult.night
      - file:         caller uses _load_osc_pkl_as_pee to recover plain P_ee

    Returns (kde_p, exposure_dict) matching normalize_true_weights expectations.
    """
    ...
    for config in configs:
        ...
        for name, tree in product(configs[config], trees):
            ...
            if "PARTICLE_TYPE" not in params or params["PARTICLE_TYPE"] is None:
                ...
                # ── Pre-compute oscillation result once per (config, name, tree) ──────
                # All three backends converge on (pee_2d, n_centers, nadir_pdf) so the
                # KDE fitting loop below is backend-agnostic.
                _pee_2d_cache    = None
                _n_centers_cache = None
                ...
                if "osc" in params.get("DEFAULT_SIGNAL_WEIGHT", []):
                    ...
                    if _osc_backend != "file":
                        ...
                        _osc_result, _nadir_pdf_cache = _precompute_osc_result(
                            _dm2, _sin13, _sin12, _osc_backend,
                        )
                        _pee_2d_cache    = _osc_result.night.values
                        _n_centers_cache = _osc_result.night.index.values
                    else:
                ...
                for (comp, label), osc in product(
                    ...
                    if osc == "osc":
                        if _pee_2d_cache is None:
                            rprint(
                                f"[red][ERROR][/red] No oscillation data available — "

...
from .plotting import format_coustom_plotly
from .io import get_branches2use, get_bkg_config, save_figure
...
def rebin_hist2d(
    x: np.ndarray, y: np.ndarray, z: np.ndarray, rebin: Optional[int]
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """
    ...
    if isinstance(rebin, int):
        ...
        # Note: z is shape (len(y), len(x)), not (len(x), len(y))
        new_z = (
            z[: len(y) // rebin * rebin, : len(x) // rebin * rebin]
            .reshape(len(y) // rebin, rebin, len(x) // rebin, rebin)
            .sum(axis=(1, 3))
        )
...
def rebin_df_columns(
    ...
    It returns a new dataframe with an additional column with the counts per energy.
    """
...
def npy2df(
    ...
    data = {
        branch: (
            run[tree][branch].tolist()
            if hasattr(run[tree][branch], "tolist")
            else run[tree][branch]
        )
...
def dict2df(run: dict, debug: bool = False) -> list[pd.DataFrame]:
    ...
    for branch in branches:
        ...
        for key in run[branch].keys():
            ...
            if debug:
                rprint(f"[magenta] --- Dataframe for key {key} created\n[/magenta]")
                rprint(f"{df[key]} \n")
        df_list.append(df)
...
def merge_df(
    df1: pd.DataFrame, df2: pd.DataFrame, label1: str, label2: str, debug: bool = False
) -> pd.DataFrame:
    """
...
def reorder_df(
    df: pd.DataFrame, info: dict, bkg_dict: dict, color_dict: dict, debug: bool = False
...
def generate_truth_dataframe(
    run: dict, info: dict, fullname: bool = True, debug: bool = False
) -> pd.DataFrame:
    """
...
def calculate_mean_truth_df(
    truth_gen_df: pd.DataFrame, debug: bool = False
) -> pd.DataFrame:
    mean_truth_df = (
...
def calculate_pileup_df(
    mean_truth_df: pd.DataFrame, info: dict, factor: float = 1, debug: bool = False
) -> pd.DataFrame:

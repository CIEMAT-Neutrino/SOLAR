...
true, reco, filter_idx = compute_root_workflow(
    user_input, info, data_filter, workflow="ANALYSIS", debug=user_input["debug"]
)
print_colored(
...
count_truth_df = get_truth_count(
    run, info, config, {config: user_input["root_file"][0]}, debug=False
)
...
for jdx, name in enumerate(user_input["root_file"]):
    ...
    for kdx, gen in enumerate(gen_list):
        filters = compute_solarnuana_filters(
            ...
            gen,
            filter_list=["Primary"],
            params={"FIDUTIAL_FACTOR": 0.075, "MIN_CL_E": 5, "MAX_CL_E": 20},
            cummulative=True,
            debug=False,
        )

...
def _project_1d_to_2d(hist_1d, oscillation_df):
...
for idx, cut in enumerate(cut_entries):
    ...
    for bkg in background_samples:
        ...
        component_energy = np.asarray(this_df["Energy"].values[0], dtype=float)
    ...
    bkg_hist = _project_1d_to_2d(total, oscillation_df)
    smoothed_bkg_hist = _project_1d_to_2d(total_smoothed, oscillation_df)

...
for config in configs:
    ...
    for name, key in product(configs[config], cut_keys):
        ...
        if args.background:
            data_path = f"/pnfs/ciemat.es/data/neutrinos/DUNE/SOLAR/SENSITIVITY/{config}/{args.name}/{args.folder.lower()}/{energy}/results/signal_{100*args.signal_uncertainty:.0f}%_and_background_{100*args.background_uncertainty:.0f}%"
        else:
            data_path = f"/pnfs/ciemat.es/data/neutrinos/DUNE/SOLAR/SENSITIVITY/{config}/{args.name}/{args.folder.lower()}/{energy}/results/signal_{100*args.signal_uncertainty:.0f}%_only"

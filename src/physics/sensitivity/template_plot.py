...
nadir_axis = load_oscillation_axis()

# Panel 4: standalone signal template T^sig(cos η, E_reco)
if signal_template is not None:
    _sig_tmpl_fig = make_subplots(rows=1, cols=1)
    _sig_tmpl_fig.add_trace(go.Heatmap(
    ...
    _sig_tmpl_fig = format_coustom_plotly(
        _sig_tmpl_fig,
        title=(
            f"Signal Template T^sig {args.config} {args.energy} "
...
# Panel 6: standalone background template T^bkg(cos η, E_reco)
if background_template is not None:
    _bkg_tmpl_fig = make_subplots(rows=1, cols=1)
    _bkg_tmpl_fig.add_trace(go.Heatmap(
    ...
    _bkg_tmpl_fig = format_coustom_plotly(
        _bkg_tmpl_fig,
        title=(
            f"Background Template T^bkg {args.config} {args.energy} "
            f"NHits{nhits_value} OpHits{ophits_value} AdjCl{adjcl_value}"
        ),
        tickformat=(".1f", ".1f"),
    )

...
rprint(
    ...
    f"({len(_study_labels_to_run)} study × {len(args.config)} config × "
    f"{len(args.signal)} signal × {len(args.energy)} energy)"
)
    ...
    for config, name, energy in product(args.config, args.signal, args.energy):
        ...
        if args.oscillation_backend == "file":
            ...
            if not os.path.exists(_pkl):
                ...
                rprint(f"[yellow][WARNING][/yellow] Skipping {config} {name} {energy}: {_msg}")
                continue
        ...
        save_figure(
            ...
            subfolder=_save_subfolder,
            filename=f"Oscillogram_{energy}",
            rm=args.rewrite, debug=args.plot,
        )
        ...
        save_figure(
            ...
            subfolder=_save_subfolder,
            filename=f"Oscillogram_NadirWeighted_{energy}",
            rm=args.rewrite, debug=args.plot,
        )
        ...
        save_figure(
            ...
            subfolder=_save_subfolder,
            filename=f"Oscillogram_NadirProjection_{energy}",
            rm=args.rewrite, debug=args.plot,
        )
        ...
        # Single combination keeps the hard fail: the caller asked for one thing and
        # must be told it is missing. A batch is expected to straddle variants whose
        # Ref pkls do not all exist yet, so there we warn and move on rather than
        # abandoning the combinations that would have succeeded.
        try:
            _true_energy  = _load_ref(f"AnalysisEnergy_{energy}_Ref").astype(float)
            _reco_energy  = _load_ref(f"AnalysisData_{energy}_Ref").astype(float)
            _base_weights = _load_ref(f"AnalysisWeights_{energy}_Ref").astype(float)
            _fid_mask     = _load_ref(
                f"FiducializationMask_{energy}_{args.analysis.upper()}"
            ).astype(bool)
        except FileNotFoundError as _exc:
            if not _batch_mode:
                raise SystemExit(str(_exc))
            rprint(
                f"[yellow][WARNING][/yellow] Skipping signal_1d for {config} {name} "
                f"{energy} study={_study_name}: {_exc}"
            )
            continue
        ...
        _fig_1d.update_xaxes(title_text=f"Reconstructed Energy — {energy} (MeV)")
        _fig_1d.update_yaxes(title_text="Events / (kt·yr · MeV)")
        save_figure(
            _fig_1d, save_path, config=config, name=name,
            subfolder=_save_subfolder,
            filename=f"Signal1D_{energy}_FidOnly",
            rm=args.rewrite, debug=args.plot,
        )

        rprint(
            f"[green][OK][/green] {config} {name} {energy} study={_study_name} — "

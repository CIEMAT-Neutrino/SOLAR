...
run = compute_reco_workflow(
    run, configs, workflow="ANALYSIS", rm_branches=False, debug=user_input["debug"]
)
...
for jdx, config in enumerate(configs):
    ...
    calibration_info = read_input_file(
    ...
    run["Reco"]["GammaE"] = 1e3 * np.sum(
        np.sum(
            run["Reco"]["TMarleyP"] * [run["Reco"]["TMarleyPDG"][:, :] == 22], axis=0
        ),
        axis=1,
    )
    run["Reco"]["NeutronP"] = 1e3 * np.sum(
        np.sum(
            run["Reco"]["TMarleyP"] * [run["Reco"]["TMarleyPDG"][:, :] == 2112], axis=0
        ),
        axis=1,
    )
    ...
    run = compute_reco_energy(
        run, configs, params={}, rm_branches=False, debug=user_input["debug"]
    )
    ...
    fig.update_layout(
        title="True Neutrino Energy vs. Reco Visible Energy",
        height=800,
        width=1600,
    ...
    print_colored(
        "-> Saved reco energy plot to ../images/calibration/%s_calibration/%s_reco_energy.png"
        % (config, config),
        "SUCCESS",
    )

...
run = compute_reco_workflow(
    run, configs, workflow="ANALYSIS", debug=user_input["debug"]
)
...
for idx, energy_bin in enumerate(energy_centers):
    ...
    hist, bin_edges = np.histogram(
        run["Reco"][info["DEFAULT_ANALYSIS_ENERGY"][0]][filter2], bins=energy_edges
    )
    ...
    fig, popt, perr = get_hist1d_fit(
        run["Reco"][info["DEFAULT_ANALYSIS_ENERGY"][0]][filter2],
        energy_edges,
        fig,
        1,
        1,
    ...
    if not os.path.exists(
        "../images/calibration/%s_calibration/%s_calibration_mono/" % (config, config)
    ):
        os.makedirs(
            "../images/calibration/%s_calibration/%s_calibration_mono/"
            % (config, config)
        )
    fig.write_image(
        "../images/calibration/%s_calibration/%s_calibration_mono/%s_calibration_%.2f.png"
        % (config, config, config, energy_bin),
        width=2400,
        height=1080,
    )
    print_colored(
        "-> Saved images to ../images/calibration/%s_calibration/%s_calibration_mono/%s_calibration_%.2f.png"
        % (config, config, config, energy_bin),
        "SUCCESS",
    )
    ...
    with open(
        ...
        + "_calib/"
        + config
        + "_{:.2f}".format(energy_bin)
        + "MeV_energy_calibration.txt",
        "w",
    ) as f:
...
smearing_df.to_pickle(
    "../config/" + config + "/" + config + "_calib/" + config + "_smearing.pkl"
)
norm_smearing_df.to_pickle(
    "../config/" + config + "/" + config + "_calib/" + config + "_norm_smearing.pkl"
)
...
fig.write_image(
    "../images/calibration/%s_calibration/%s_smearing.png" % (config, config),
    width=2400,
    height=1080,
)
print_colored(
    "-> Saved images to ../images/calibration/%s_calibration/%s_smearing.png"
    % (config, config),
    "SUCCESS",
)
# fig.show()
...
fig.write_image(
    "../images/calibration/%s_calibration/%s_smearing_hist.png" % (config, config),
    width=2400,
    height=1080,
)
print_colored(
    "-> Saved images to ../images/calibration/%s_calibration/%s_smearing_hist.png"
    % (config, config),
    "SUCCESS",
)
# fig.show()

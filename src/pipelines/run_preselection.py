"""
run_preselection.py — Preselection Efficiency Pipeline
=======================================================
Runs production, efficiency, and clustering preselection diagnostics.
...
# ── CLI ──────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Run preselection efficiency diagnostics")
parser.add_argument(
    "--config",
    type=str,
    nargs="+",
...
rprint(f"\n[bold]Preselection efficiency diagnostics pipeline[/bold]")
...
for config, name in product(args.config, args.name):
    ...
    run_python_command(build_command("src/physics/detector/preselection/01_production.py", base_args))
    run_python_command(build_command("src/physics/detector/preselection/02_efficiency.py", base_args))
    run_python_command(build_command("src/physics/detector/preselection/03_clustering.py", base_args))

rprint(f"\n[bold green]Preselection pipeline complete.[/bold green]")

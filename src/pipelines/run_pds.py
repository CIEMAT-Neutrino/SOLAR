"""
run_pds.py — PDS (OpFlash) Diagnostics Pipeline
================================================
Runs OpFlash matching and efficiency diagnostics.
...
rprint(f"\n[bold]PDS diagnostics pipeline[/bold]")
...
for config, name in product(args.config, args.name):
    ...
    run_python_command(build_command("src/physics/detector/pds/01_opflash.py",         base_args))

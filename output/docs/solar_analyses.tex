...
For the hep signal component the corresponding quantity uses
$\srel^{\mathrm{sig}} = 30\%$ (HEP) or $4\%$ (Sensitivity).
In the HEP analysis ${}^8$B is treated as a background component and uses
$\srel^{{}^8\mathrm{B}} = \srel^{\mathrm{bkg}} = 2\%$
(Section~\ref{sec:hep_signal}).
...
The \textbf{asymmetry signal} at scale factor $\theta_s$ (Section~\ref{sec:dn_band}) is:
\begin{equation}
...
Under the signal hypothesis (asymmetry scale $\theta_s$), the Asimov
...
\medskip
\noindent\textbf{Gaussian constraint on the asymmetry amplitude.}
The asymmetry amplitude $\theta_s$ is treated as a constrained nuisance:
$\theta_s \sim \mathcal{N}(1, \epstot^2)$, where $\epstot$ is the total
band (Eq.~\eqref{eq:dn_band}).
For each band scenario $\theta_s \in \{1+\epstot,\;1,\;1-\epstot\}$ the
penalised test statistic is:
\begin{equation}
  q^{\mathrm{pen}}(\theta_s)
  = \sum_{i \ge i_{\mathrm{th}}} q_i(\theta_s)
    - \left(\frac{\theta_s - 1}{\epstot}\right)^{\!2}.
  \label{eq:dn_asimov_penalised}
\end{equation}
...
\end{equation}
At the nominal point $\theta_s=1$ the penalty vanishes and
Eq.~\eqref{eq:dn_asimov_global} reduces to the raw LLR sum.
At $\theta_s = 1\pm\epstot$ the penalty equals $1$, deflating the raw
sum by exactly $1\,\sigma^2$ relative to the nominal curve.
This correctly reflects the prior information on the asymmetry scale: the
off-nominal band significances are not free upper/lower bounds but
penalised estimates constrained by the prior uncertainty.

\medskip
\noindent\textbf{Asimov as the primary cut-optimization metric.}
The nominal-band smoothed Asimov significance $\ZA(\theta_s{=}1)$ is the
primary metric for selecting the optimal topological cuts: the cut that
minimises the exposure needed to reach $\ZA \ge 2\sigma$ is retained.
The Gaussian significance is computed in parallel as a diagnostic output
but does not drive cut selection.
The output columns \texttt{Sigma2}/\texttt{Sigma3} and
\texttt{AsimovSigma2}/\texttt{AsimovSigma3} are identical (both record
the Asimov-based exposure thresholds); \texttt{AsimovSigma2} is an alias
retained for backward compatibility with older result readers.
...
\medskip
\noindent\textbf{Gaussian estimators.}
The scale $\theta_s$ enters the signal multiplicatively
(Eq.~\eqref{eq:dn_signal}),
leaving the background unchanged.
Three curves (upper, nominal, lower) are produced for each Gaussian estimator.

\medskip
\noindent\textbf{Asimov estimator.}
For the Asimov LLR the asymmetry amplitude is treated as a constrained
nuisance (Section~\ref{sec:dn_asimov}): $\theta_s$ enters both the signal
and the Gaussian penalty term $(\theta_s-1)^2/\epstot^2$ in
Eq.~\eqref{eq:dn_asimov_penalised}.
The three Asimov curves are therefore deflated from their unconstrained
values by the penalty, correctly encoding the prior information on the
asymmetry scale.
...
\medskip
\noindent\textbf{Component uncertainty assignment.}
The analysis distinguishes three uncertainty classes:
\begin{itemize}[leftmargin=*]
  \item \textbf{Pure backgrounds} ($\gamma$, $n$, radiological):
    $\srel^c = \srel^{\mathrm{bkg}}$ (\texttt{--background\_uncertainty}, default 2\%).
  \item \textbf{${}^8$B solar (oscillated background)}:
    also assigned $\srel^{{}^8\mathrm{B}} = \srel^{\mathrm{bkg}}$ (2\%).
    Although ${}^8$B is an oscillated solar signal, at the energies probed
    by HEP (14--30\,MeV) its rate is well constrained and it enters as a
    known background.  Treating it with background uncertainty suppresses the
    absorbing-plateau problem that would arise from inflating its normalization
    freedom to the 30\% level.
  \item \textbf{hep signal}: $\srel^{\mathrm{hep}} = \srel^{\mathrm{sig}}$
    (\texttt{--signal\_uncertainty}, default 30\%).
    The \texttt{--signal\_uncertainty} flag and the \texttt{unc\_sig} study
    scan affect \emph{only} the hep component.
\end{itemize}

\subsection{Background Error Aggregation}
\label{sec:hep_bkg_error}

Each background component $c \in \{\gamma, n, \mathrm{rad}, {}^8\mathrm{B}\}$
contributes an absolute uncertainty $\sigma_i^c$ (Eq.~\eqref{eq:bin_error})
computed with $\srel^c = \srel^{\mathrm{bkg}}$.
...
Signal
  & $\si{=}\Ecal\theta_s(r_i^{\rm night}{-}r_i^{\rm day})$
  & $\si{=}\Ecal r_i^{\rm hep}$
  & $\Tsig(\vec\theta)$ \\
Goal
...
Significance type
  & Gaussian (diagnostic), two-sample Asimov (primary)
  & Gaussian, Asimov, PL
  & $\chi^2$ surface \\
Nuisances
...
\multicolumn{6}{l}{\textit{Day-Night}} \\
 ...
 & \texttt{RawAsimov}                & Raw    & None & Two-sample LLR (penalised) & No \\
 & \texttt{Asimov}                   & Smooth & None & Two-sample LLR (penalised) & No \\
 & \texttt{Sigma2} / \texttt{AsimovSigma2}  & Smooth & None & Asimov exposure threshold (alias pair) & --- \\
\midrule

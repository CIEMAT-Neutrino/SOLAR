...
data, filter_idx = compute_root_workflow(user_input, info, data_filter, workflow="CALIBRATION", debug=user_input["debug"])
...
fig, top_bottom_popt, top_bottom_perr = get_hist2d_fit(data["ElectronE"][filter_idx],data["TNuE"][filter_idx],acc,fig,1,1,trimm=15,threshold=0.3,spec_type="top+bottom",func_type="linear",debug=user_input["debug"])
...
fig.update_layout(title="True Neutrino Energy vs. Reco Visible Energy",width=2400, height=1080,template="presentation",coloraxis=dict(colorscale="Turbo",colorbar=dict(title="Counts")),

...
default_dict = {}
user_input = initialize_macro("04Computing",["config_file","root_file","rewrite","debug"],default_dict=default_dict, debug=True)
...
############################
    for j in range(len(tree.AdjOpFlashR)):
        if tree.AdjOpFlashR.at(j) > params["MAX_FLASH_R"]: continue
        if tree.AdjOpFlashPE.at(j) < params["MIN_FLASH_PE"]: continue
        if tree.AdjOpFlashPE.at(j)/tree.AdjOpFlashMaxPE.at(j) < tree.AdjOpFlashR.at(j)*params["RATIO_FLASH_PEvsR"]: continue
...
############################
    if info["GEOMETRY"][0] == "hd":
        if  tree.TPC%2 == 0:
            try: data["RecoX"][i] = abs(data[params["DEFAULT_RECOX_TIME"]][i])*(info["DETECTOR_SIZE_X"][0]/2)/info["EVENT_TICKS"][0]
            except KeyError: data["RecoX"][i] = abs(tree.Time)*(info["DETECTOR_SIZE_X"][0]/2)/info["EVENT_TICKS"][0]
        else:
            try: data["RecoX"][i] = -abs(data[params["DEFAULT_RECOX_TIME"]][i])*(info["DETECTOR_SIZE_X"][0]/2)/info["EVENT_TICKS"][0]
            except KeyError: data["RecoX"][i] = -abs(tree.Time)*(info["DETECTOR_SIZE_X"][0]/2)/info["EVENT_TICKS"][0]
    ...
    if info["GEOMETRY"][0] == "vd":
        try: data["RecoX"][i] = -abs(data[params["DEFAULT_RECOX_TIME"]][i])*info["DETECTOR_SIZE_X"][0]/info["EVENT_TICKS"][0] + info["DETECTOR_SIZE_X"][0]/2
        except KeyError: data["RecoX"][i] = -abs(tree.Time)*info["DETECTOR_SIZE_X"][0]/info["EVENT_TICKS"][0] + info["DETECTOR_SIZE_X"][0]/2
    
############################
# Energy Computation
############################
    try: data["Correction"][i]  = np.exp(np.abs(data[params["DEFAULT_ENERGY_TIME"]][i])/corr_popt[1])
    except KeyError: data["Correction"][i]  = np.exp(np.abs(tree.Time)/corr_popt[1])
...
for key in data.keys():
    ...
    if os.path.isfile(info["PATH"][0]+info["NAME"][0]+user_input["root_file"][0]+"/Reco/"+key+".npy") and not user_input["rewrite"]:
        print_colored("File %s already exists, skipping..."%key,"WARNING")
        continue
    
    np.save(info["PATH"][0]+info["NAME"][0]+user_input["root_file"][0]+"/Reco/"+key+".npy", data[key])

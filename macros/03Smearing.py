...
for idx,energy_bin in enumerate(energy_centers):
    ...
    if not os.path.exists("../plots/calibration/%s_calibration/%s_calibration_mono/"%(config,config)): os.makedirs("../plots/calibration/%s_calibration/%s_calibration_mono/"%(config,config))
    fig.write_image("../plots/calibration/%s_calibration/%s_calibration_mono/%s_calibration_%.2f.png"%(config,config,config,energy_bin), width=2400, height=1080)
    print_colored("-> Saved plots to ../plots/calibration/%s_calibration/%s_calibration_mono/%s_calibration_%.2f.png"%(config,config,config,energy_bin),"SUCCESS")
    
    with open("../config/"+config+"/"+config+"_calib/"+config+"_{:.2f}".format(energy_bin)+"MeV_calib_config.txt",'w') as f:
...
fig.write_image("../plots/calibration/%s_calibration/%s_smearing.png"%(config,config), width=2400, height=1080)
print_colored("-> Saved plots to ../plots/calibration/%s_calibration/%s_smearing.png"%(config,config),"SUCCESS")
# fig.show()
...
fig.write_image("../plots/calibration/%s_calibration/%s_smearing_hist.png"%(config,config), width=2400, height=1080)
print_colored("-> Saved plots to ../plots/calibration/%s_calibration/%s_smearing_hist.png"%(config,config),"SUCCESS")
# fig.show()
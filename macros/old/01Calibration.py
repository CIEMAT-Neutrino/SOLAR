...
default_dict = {}
user_input = initialize_macro("00Process",["config_file","root_file","rewrite","debug"],default_dict=default_dict, debug=True)
...
if user_input["debug"]: print_colored("Filtering %d events"%np.sum(filter1),"SUCCESS")
fig, corr_popt, perr = get_hist2d_fit(np.abs(run["Reco"]["Time"][filter1]),(run["Reco"]["Charge"]/(1e3*run["Reco"]["TMarleyE"][:,2]))[filter1],acc,fig,1,1,func_type="exponential",debug=user_input["debug"])
...
fig, reco_popt, perr = get_hist2d_fit(1e3*run["Reco"]["TMarleyE"][:,2][filter1],run["Reco"]["Energy"][filter1],acc,fig,2,1,func_type="linear",debug=user_input["debug"])
fig, res_popt, perr = get_hist1d_fit((run["Reco"]["Charge"]*run["Reco"]["Correction"]/(corr_popt[0]*1e3*run["Reco"]["TMarleyE"][:,2]))[filter1],2*acc,fig,2,2,func_type="gauss",debug=user_input["debug"])
